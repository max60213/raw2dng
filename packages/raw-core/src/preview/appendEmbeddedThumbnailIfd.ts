import type { EmbeddedPreview } from "./createEmbeddedPreview";

const TIFF_MAGIC = 42;
const IFD0_OFFSET = 4;

export function appendEmbeddedThumbnailIfd(dngBytes: Uint8Array, preview: EmbeddedPreview): Uint8Array {
  ...
  const bitsPerSample = packShorts([8, 8, 8]);
  const bitsOffset = dngBytes.byteLength;
  const imageOffset = padEven(bitsOffset + bitsPerSample.byteLength);
  const ifdOffset = padEven(imageOffset + preview.imageData.byteLength);

  const entries = [
    tag(254, 4, 1, packLongs([1])),
    tag(256, 4, 1, packLongs([preview.width])),
    tag(257, 4, 1, packLongs([preview.height])),
    tag(258, 3, 3, bitsPerSample),
  ...
  output.set(bitsPerSample, bitsOffset);
  output.set(preview.imageData, imageOffset);
  ...
  entries.forEach((entry, index) => {
    ...
    const offset = entry.id === 258 ? bitsOffset : imageOffset;
    ifdView.setUint32(cursor + 8, offset, true);
  });
